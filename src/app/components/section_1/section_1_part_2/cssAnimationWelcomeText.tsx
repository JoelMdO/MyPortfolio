...
const CssAnimationWelcomeText: React.FC<CssAnimationWelcomeTextProps> = ({
  setIsAnimationCSSEnded,
}) => {
  const [dots, setDots] = useState<{ x: number; y: number }[]>([]);
  const [shapeDots, setShapeDots] = useState<
    { x: number; y: number; color: string }[]
  >([]);
  const [animationPhase, setAnimationPhase] = useState<"dots" | "shape">(
    "dots"
  );
      ...
      <div className="absolute bottom-0 right-0 m-1.5">
        ...
        >
          ...
          <path
            ...
            stroke={
              animationPhase === "shape" ? "url(#neonGradient)" : "transparent"
            }
            strokeWidth="3"
            fill="transparent"
